...
def foo(goal: int, total: int, remaining: int) -> bool:
    if len(remaining) == 1:
        return total + remaining[0] == goal or total * remaining == goal
    else:
        return foo(goal, total*remaining[0], remaining[1:]) or foo(goal, total+remaining[0], remaining[1:])
